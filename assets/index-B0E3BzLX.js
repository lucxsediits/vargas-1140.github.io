...
Acesse o painel para ver detalhes.`;navigator.clipboard.writeText(re).then(()=>alert("Relatório copiado!"))},w=()=>{const _=new Date().toLocaleDateString("pt-BR"),Q=new Date().toLocaleTimeString("pt-BR"),re=Object.values(n).filter(W=>W.notes&&W.notes.trim().length>0).sort((W,J)=>parseInt(W.id)-parseInt(J.id));let le=`
      ...
      <body>
        <table>
          ...
          <tr><td colspan="24" class="sub-header">Exportado em: ${_} às ${Q}</td></tr>
    ...
    `,["pronto","facil","dificil","nao-verificado"].forEach(W=>{const J=A[W],ce=fe>0?(J/fe*100).toFixed(1):"0.0",Ue=Jt[W].excelColor;le+=`
      ...
      `}),re.length>0&&(le+=`
        <tr><td colspan="24" style="height: 20px;"></td></tr>
        <tr><td colspan="10" class="notes-header">📝 DETALHAMENTO DE OBSERVACÕES (${re.length})</td><td colspan="14"></td></tr>
        <tr>
